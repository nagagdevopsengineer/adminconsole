...
// ** Store & Actions
import { getData, getVaccinationStatus } from '../store'
...
import HelperExportExcel from "@src/views/components/export-excel/helper_excel.js"
...
const CustomHeader = ({ store, toggleSidebar, handlePerPage, rowsPerPage, handleFilter, searchTerm }) => {
  ...
  const hideAF = () => setAdvanceFilter(false)
    ...
    const handleStatusFilter = e => {
     ...
     // ** Function to handle name and status filter
    const searchFilterDAta = (event) => {
      event.preventDefault()
      console.log("hlo")
      // setFilteredDataName(searchName)
      // console.log("h2")
      // setFilteredDataStatus(searchStatus)
      console.log("h3")
      dispatch(
        getData({
          q: searchName,
      ...
      <Row>
        ...
        >
          ...
          <div className='d-flex align-items-center table-header-actions'>
            <HelperExportExcel csvData={store.data} fileName='helper' />
...
const HelpersList = () => {
  ...
  const handleFilter = val => {
    console.log("handle filter value", val)
    setSearchTerm(val)
  }
      ...
      <Card className='overflow-hidden'>
        <div className='react-dataTable'>
          <DataTable
            ...
            data={dataToRender()}
           // pageCount={Math.ceil(dataToRender().length / 2) || 1}
            subHeaderComponent={
              <CustomHeader
                store={store}

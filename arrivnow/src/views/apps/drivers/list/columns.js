...
import { getDriver, deleteDriver } from '../store'
...
// ** Renders Driver Columns
const renderDriver = row => {
...
export const columns = () => {
  ...
  const handleDeleteAction = (row_id) => {
    return MySwal.fire({
      ...
      if (result.value) {
        store.dispatch(deleteDriver(row_id))
    ...
    cell: row => (
      <div className='d-flex justify-content-left align-items-center'>
        ...
        <div className='d-flex flex-column'>
          <Link
            to={`/apps/drivers/view/${row.id}`}
            className='user_name text-truncate text-body'
            onClick={() => store.dispatch(getDriver(row.id))}
          >
  ...
  },
  {
    ...
    sortable: true,
    sortField: 'mobile',
    selector: row => row.attributes.mobile,
  ...
  },
  {
    ...
    minWidth: '230px',
    sortable: true,
    sortField: 'vaccinationstatus',
    selector: row => row.attributes.vaccinationstatus,
    cell: row => <span className='text-capitalize'>{row.attributes.vaccinationstatus}</span>
  },
  {
    name: 'Status',
    minWidth: '138px',

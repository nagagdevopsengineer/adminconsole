...
const UserView = () => {
  ...
  // ** Get suer on mount
  useEffect(() => {
    dispatch(getRoster(parseInt(id)))
  }, [dispatch])
  ...
  return store.selectedRoute !== null && store.selectedRoute !== undefined ? (
    <div className='app-user-view'>
      <Row>
        ...
        <CustomMap
          ...
          directions={[
            {
            destination: {query: store.selectedRoute.starting_point ?? ''},
            origin: {query: store.selectedRoute.ending_point ?? ''},
    ...
    <Alert color='danger'>
      ...
      <div className='alert-body'>
        Bus with id: {id} doesn't exist. Check list of all Buses: <Link to='/apps/roster/list'>Routes  </Link>
      </div>
    </Alert>
  )
}

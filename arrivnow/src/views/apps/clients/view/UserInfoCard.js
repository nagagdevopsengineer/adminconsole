...
const UserInfoCard = ({ selectedUser }) => {
  ...
  const gst_image = (selectedUser.attributes.gstimage.data) ? selectedUser.attributes.gstimage.data.attributes.url : ''
  const pan_image = (selectedUser.attributes.pancardimage.data) ? selectedUser.attributes.pancardimage.data[0].attributes.url : ''

  // ** render user img
  const renderUserImg = () => {
    const user_image = (selectedUser.attributes.logo.data) ? selectedUser.attributes.logo.data[0].attributes.url : ''
  ...
  const handleSuspendedClick = () => {
    return MySwal.fire({
      title: 'Are you sure?',
      text: "You want to reject this client!",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Yes, Rejected client!',
      customClass: {
      ...
      if (result.value) {
        dispatch(
          updateClient({
            id: selectedUser.id,
            data: {name: selectedUser.attributes.name,
  ...
  const handleApprovedClick = () => {
    ...
    return MySwal.fire({
      title: 'Are you sure?',
      text: "You want to approve this client!",
      icon: 'success',
      showCancelButton: true,
        ...
        <CardBody className='px-1'>
          <div className='row'>
            <div className='user-avatar-section col-md-12 col-lg-12 col-xl-12 mb-4 uas_back pad_10x'>
              ...
              <h4 className='fw-bolder border-bottom pb-50 mb-1'>Client Info</h4>
            {selectedUser !== null ? (
              <div className='row'>
                ...
                <div className='mb-75 col-4'>
                  <div className='fw-bolder me-25'>Company Name</div>
                  <div className='usr_ipt_cust'>{selectedUser.attributes.companyname ?? ''}</div>
                </div>
                <div className='mb-75 col-4'>
               ...
               {/*} <div className='mb-75 col-4'>
                  ...
                  <div className='usr_ipt_cust'>{selectedUser.company ?? ''}</div>
                </div> */ }
                <div className='mb-75 col-4'>
                  <div className='fw-bolder me-25'>Registration Number</div>
                  <div className='usr_ipt_cust'>{selectedUser.attributes.registration_number ?? ''}</div>
               </div>
                
                <div className='mb-75 col-4'>
                  <div className='fw-bolder me-25'>GST Number </div>
                  <div className='usr_ipt_cust'>{selectedUser.attributes.gstnumber}</div>
                </div>
                <div className='mb-75 col-4'>

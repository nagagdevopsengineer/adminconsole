...
export const columns = () => {
  ...
  const handleDeleteAction = (row_id) => {
    return MySwal.fire({
      ...
      if (result.value) {
        store.dispatch(deleteEmployee(row_id))
  ...
  },
  {
    ...
    cell: row => <span className='text-capitalize'>{row.attributes.contact}</span>
  },
  {
    name: 'Status',
    minWidth: '138px',
    sortable: true,
    sortField: 'status',
    selector: row => row.attributes.is_activated,
    cell: row => (
      <Badge className='text-capitalize' color={statusObj[row.attributes.is_activated ? 'Approved' : 'pending']} pill>
        {row.attributes.is_activated ? 'Approved' : 'Pending'}
      </Badge>
    )
  },
  {
